...
### Sentiment Analysis
Since the quotation data has no label information, we cannot adopt the supewrvised machine learning techniques. Here we apply the pretrained model from Transformers to finish the sentiment analysis task. Using the pretrained model, we obtain the positive or negative label of each quotations. This method is a time-consuming task. Therefore, we only do this part toward UK people and save the output so that it is easier for us to load it later.
# Proposed timeline :
